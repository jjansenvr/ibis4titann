...
public class CmisSender extends SenderWithParametersBase {
	...
	public void configure() throws ConfigurationException {
		...
		if (!getBindingType().equalsIgnoreCase("atompub")
				...
				&& !getBindingType().equalsIgnoreCase("browser")
				) {
			throw new ConfigurationException("illegal value for bindingType ["
					+ getBindingType()
					+ "], must be 'atompub' or 'webservices' or 'browser'");
		}
	...
	private String sendMessageForActionCreate(String correlationID,
		...
		if (StringUtils.isNotEmpty(fileInputStreamSessionKey)) {
			...
			//MTOM checks for the size of the input file. If the file is less than 100 kb, non streaming is uses.
			//If this is the case a ByteArrayInputStream is used and we switch to the  non streaming inputstream for cmis
            if( prc.getSession().get(getFileInputStreamSessionKey()) instanceof ByteArrayInputStream)
            {
            	inputStream = (ByteArrayInputStream) prc.getSession().get(getFileInputStreamSessionKey());
    			//String fileContent = (String) prc.getSession().get(getFileInputStreamSessionKey());
