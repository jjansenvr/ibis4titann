...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<xsl:variable name="Class">
		<xsl:call-template name="lookup">
			<xsl:with-param name="lookupValue" select="UpdateDocumentAttributes_1_Action/DocumentClass"/>
	...
	<cmis>
		<id>
		<xsl:choose>
		<xsl:when test="$objectId"><xsl:value-of select="$objectId"/></xsl:when>
		<xsl:otherwise><xsl:value-of select="UpdateDocumentAttributes_1_Action/ID"/></xsl:otherwise>
		</xsl:choose>
		</id>
		<xsl:apply-templates select="UpdateDocumentAttributes_1_Action/MetaData">
	...
	<xsl:param name="Class"/>
		<properties>
			...
			<property name="DocumentName"><xsl:value-of select="DocumentTitle"/></property>
			<property name="ShortTitle"><xsl:value-of select="normalize-space( replace( replace( replace( substring(ArchiveDocument_1_Action/Document/MetaData/DocumentTitle,1,44) ,'%' ,' ') ,'/' ,' '),'\\' ,' '))" /></property>
			</xsl:if> 
			...
			<xsl:if test="Source"><property name="Source"><xsl:value-of select="Source"/></property></xsl:if>
			...
			<xsl:if test="../SetRetention/ExternalEventTriggerDate">
				<property name="ExternalEventTriggerDate" type="datetime" formatString="yyyy-MM-dd"><xsl:value-of select="format-date(../SetRetention/ExternalEventTriggerDate,'[Y0001]-[M01]-[D01]')"/></property>
			</xsl:if>
			<xsl:if test="../SetRetention/RetentionPolicy">
				<property name="RetentionPolicy"><xsl:value-of select="../SetRetention/RetentionPolicy"/></property>
			</xsl:if>
			<xsl:if test="../SetRetention/RetentionTrigger">
				<property name="RetentionTrigger"><xsl:value-of select="../SetRetention/RetentionTrigger"/></property>
			</xsl:if>
		</properties>
	</xsl:template>
