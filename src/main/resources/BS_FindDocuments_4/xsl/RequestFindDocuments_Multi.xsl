			...
			<xsl:otherwise>
				<query>
					<statement><xsl:text>SELECT * FROM </xsl:text>
						...
						<xsl:if test="IncludeMedicalDocuments='true'"><xsl:text>, NN1_L_GENERIEK_MED</xsl:text></xsl:if>
						<xsl:if test="IncludeSpecialCasesDocuments='true'"><xsl:text>, NN1_L_GENERIEK_SZ</xsl:text></xsl:if>
...
<!-- 		</xsl:if> -->
		...
		<xsl:if test="string-length(ProductCode)>0">
			<searchAttribute>ProductCode=&apos;<xsl:value-of select="ProductCode"/>&apos;</searchAttribute>
		</xsl:if>
